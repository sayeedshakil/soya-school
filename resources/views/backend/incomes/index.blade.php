...
@can('income_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('backend.incomes.create') }}">
                ...
                <tbody>
                    @foreach($incomes as $key => $income)
                        <tr data-entry-id="{{ $income->id }}">
                            ...
                            <td>
                                @can('income_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('backend.incomes.show', $income->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('income_show')
                                    <a class="btn btn-xs btn-secondary" href="{{ route('backend.incomes.invoice', $income) }}">
                                        {{ trans('global.invoice') }}
                                    </a>
                                @endcan

                                @can('income_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('backend.incomes.edit', $income->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('income_delete')
                                    <form action="{{ route('backend.incomes.destroy', $income->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('income_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('backend.incomes.massDestroy') }}",
