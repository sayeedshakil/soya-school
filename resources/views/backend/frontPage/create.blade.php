...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route('backend.front-page.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="form-group {{ $errors->has('headerNameEn') ? 'has-error' : '' }}">
                <label for="headerNameEn">{{ trans('cruds.frontPage.headerNameEn') }}*</label>
                <input type="text" id="headerNameEn" name="headerNameEn" class="form-control" value="{{ old('headerNameEn') }}" required>
            ...
            <div class="form-group {{ $errors->has('headerNameBn') ? 'has-error' : '' }}">
                ...
                <input type="text" id="headerNameBn" name="headerNameBn" class="form-control" value="{{ old('headerNameBn') }}" required>
            ...
            <label for="headTeacherImage">{{ trans('cruds.frontPage.headTeacherImage') }}</label>
            <div class="form-group text-center" {{ $errors->has('headTeacherImage') ? 'has-error' : '' }} style="background-color: #f1f1f1;
                ...
                <div>
                    <img src="{{asset('backend/images/default/img12.jpg')}}" onclick="triggerClick()" style="max-height: 140px;border-radius: 3px;" id="previewImage" alt="click here to add headTeacherImage" title="click here to add headTeacherImage">
                    <input type="file" class="form-control" name="headTeacherImage" id="profileImage" style="display: none;"
                ...
                <div class="mt-2">
                    ...
                    <div class="text-muted" style="font-size: .80rem;">
                        Supported file type pdf,dox,jpg,jpeg <br>Maximum file size is 2MB
                    </div>
                </div>
            </div>
            ...
            <div class="form-group {{ $errors->has('headTeacherName') ? 'has-error' : '' }}">
                ...
                <input type="text" id="headTeacherName" name="headTeacherName" class="form-control" value="{{ old('headTeacherName') }}" required>
            ...
            <div class="form-group {{ $errors->has('latestNewsText') ? 'has-error' : '' }}">
                ...
                <textarea id="latestNewsText" class="form-control {{ $errors->has('latestNewsText') ? 'is-invalid' : '' }}" name="latestNewsText" >{{ old('latestNewsText') }}</textarea>
