...
@can('income_category_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('backend.income-categories.create') }}">
                        ...
                        <tr data-entry-id="{{ $incomeCategory->id }}">
                            ...
                            <td>
                                @can('income_category_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('backend.income-categories.show', $incomeCategory->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('income_category_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('backend.income-categories.edit', $incomeCategory->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('income_category_delete')
                                    <form action="{{ route('backend.income-categories.destroy', $incomeCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('income_category_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('backend.income-categories.massDestroy') }}",
